...
class _IncomeInputScreenState extends State<IncomeInputScreen> {
  ...
  Future<void> _fetchOriginalIncome(String docId) async {
    try {
      ...
      final String? lastId =
          original.lastRecurringIncomeTemplateId ??
          original.recurringIncomeTemplateId;
    ...
    try {
      ...
      setState(() {
        ...
        if (widget.editItem != null && _incomeCategories.isNotEmpty) {
          final matches = _incomeCategories
              .where((c) => c['id'] == widget.editItem!.categoryId)
              .toList();
          if (matches.isNotEmpty) {
            ...
            _selectedParentCategory =
                matched['parentName']?.toString() ??
                matched['parent']?.toString() ??
                '미분류';
          } else {
      ...
      if (_isRecurring) {
        if (recurringTemplateId != null) {
          ...
          await _incomeService
              .updateRecurringTemplate(lastRecurringTemplateId, {
                'isDeleted': false,
                'deletedAt': null,
                'isActive': true,
                'amount': _currentAmount,
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> parentCategories = _incomeCategories
        .map(
          (category) => (category['parentName'] ?? category['parent'] ?? '미분류')
              .toString(),
        )
        .toSet()
        .toList();
    ...
    final List<Map<String, dynamic>> childCategories =
        ...
        : _incomeCategories.where((category) {
            final parent =
                category['parentName'] ?? category['parent'] ?? '미분류';
            return parent == _selectedParentCategory;
          }).toList();

    // 💡 핵심: 대분류가 '정기'로 시작할 때만 정기수입으로 간주 ('비정기 수입'은 제외)
    final bool isRegularIncome =
        _selectedParentCategory != null &&
        _selectedParentCategory!.startsWith('정기');
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(color: const Color(0xFFD6F1E2)),
                          ),
                          child: Row(
                            ...
                            children: [
                              ...
                              ValueListenableBuilder<TextEditingValue>(
                                ...
                                builder: (context, value, _) {
                                  final amount = parseAmount(value.text);
                                  return Text(
                                    '원',
                                    style: TextStyle(
                                      fontSize: 24,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          value:
                              parentCategories.contains(_selectedParentCategory)
                              ? _selectedParentCategory
                              : null,
                          hint: const Text(
                            '대분류 선택',
                          ...
                          onChanged: parentCategories.isEmpty
                              ? null
                              : (newParent) {
                                  setState(() {
                                  ...
                                  children: [
                                    ...
                                    _sectionLabel(
                                      ...
                                      trailing: IconButton(
                                        ...
                                        onPressed: () async {
                                          final newId =
                                              await showAddSubCategoryDialog(
                                                context,
                                    ...
                                    DropdownButtonFormField<String>(
                                      ...
                                      onChanged: childCategories.isEmpty
                                          ? null
                                          : (newId) {
                                              setState(() {
                                                _selectedCategoryId = newId;
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Text(
                                  '입금일: ${_selectedDate.toLocal().toString().split(' ')[0]}',
                                  style: const TextStyle(
                                    fontSize: 14,
                                    fontWeight: FontWeight.w700,
                                    color: AppColors.ink,
